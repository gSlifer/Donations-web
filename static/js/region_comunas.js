...
function loadOptions() {
    ...
    // Load comunas for the initial region
    loadComunas(regionSelect.value, comunaSelect);
    ...
    function loadComunas(regionId, comunaSelect) {
      const region = regiones.find(r => r.id == regionId);
      if (!region) return;
      for (let i = 0; i < region.comunas.length; i++) {
        const option = document.createElement("option");
        option.text = region.comunas[i].nombre;
        option.value = region.comunas[i].id;
